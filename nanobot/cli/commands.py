...
def gateway(
    ...
    async def run():
        from nanobot.web.gateway import run_web_gateway
        from nanobot.web.gateway.server import WebGateway
        ...
        await runner.setup()
        site = web.TCPSite(runner, config.gateway.host, port)
        await site.start()
        console.print(f"[green]✓[/green] Web gateway started on http://{config.gateway.host}:{port}")
